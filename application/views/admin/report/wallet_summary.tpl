...
{* 
							...
							<select class="form-select form-select-sm mb-3" name="category" id="category">
								{* <option value="">{lang('all_transactions')}</option> *}
								{foreach $category_details as $v}
								<option value="{$v.id}" {if $post_arr['category'] == $v.id}selected{/if}>{lang($v.db_amt_type)}</option>
								{/foreach}
							</select>
							{form_error("category")}
						...
						<table class="table color-table primary-table" id="exportTable">
							<thead >
								...
								<tr>
									...
									{if $post_arr['category_name']=='level_bonus'}
									<th>{lang('level_no')}</th>  
									{/if} 
									{if $post_arr['category_name']=='global_bonus'}
									<th>{lang('Status')}</th>  
									{/if}                     
									<th>{lang('method')}</th>                                 
							...
							{* <tbody>
								...
								<tr>
									...
									{/if}     
									<td class="text-right">{$v.user_name}</td>                           
									...
									<td>{date("d-m-y", strtotime($v.date_of_submission))}</td>
									<td class="text-right"><b>{cur_format($v.user_page_total_amount)}</b></td>
								</tr>
...
<script type="text/javascript">
	$(document).ready(function(){
		var order = $('#exportTable').DataTable({
			...
			'ajax': {
				'url':'{base_url()}admin/report/get_WalletSummary_ajax',
				"type": "POST",
				"data" : {

					'category_type' : '{$post_arr['category_name']}',
			...
			'columns': [
				...
				{/if}{if $post_arr['category_name']=='global_bonus'}
				{ data: 'status' },
				{/if}
				{ data: 'category_name' },
