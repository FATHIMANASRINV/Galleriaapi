	...
	<div class="col-sm-12"> 
		...
		<div class="form-body">
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<label for="user_id" class="control-label">{lang('username')}</label>
						<select name="user_id" id="user_id" class="form-control user_name_ajax" >
							{if $post_arr['user_id']}
							<option value="{$post_arr['user_id']}" selected>{$post_arr['user_name']}</option>
							{/if}
						</select>
			...
			<div class="row">
				<div class="col-sm-4">
					<div class="form-group">
						<label for="user_name" class="control-label">{lang('sponsor_name')}</label>
						<select name="sponsor_id" id="sponsor_id" class="form-control user_name_ajax" >
							{if $post_arr['sponsor_id']}
							<option value="{$post_arr['sponsor_id']}" selected>{$post_arr['sponsor_name']}</option>
							{/if}
						</select>
	...
	$(document).ready(function(){
		var order = $('#exportTable').DataTable({
			...
			buttons: [
				...
				'ajax': {
					'url':'{base_url()}admin/report/get_UserJoining_ajax',
					"type": "POST",
					"data" : {
