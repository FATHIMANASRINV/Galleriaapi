...
class Software {
    ...
    function __construct()
    {
        ...
        $this->COMMON_PAGES = [ 'login', 'signup', 'backup', 'jsloader' , 'website','cron_jobs' ]; 
        $this->NO_LANG_CLASS = [ 'fix_issues', 'cron_jobs', 'autocomplete', 'truncate', 'backup', 'jsloader','auth','home','user' ];
        $this->NO_LOGIN_PAGES = [ 'login', 'backup',  'fix_issues', 'website' ];
        $this->NO_MODEL_CLASS_PAGES = [ 'autocomplete', 'jsloader','auth','home','user' ];
        ...
        // }
         if(isset($this->CI->session->userdata['site_logged_in']) && $this->CI->session->userdata['site_logged_in']){ 
            $this->LOGGED_IN_ARR = $this->CI->session->userdata('site_logged_in');
            $this->LANG_ID = $this->LOGGED_IN_ARR['lang_id'];

        }else{
            $this->LANG_ID = $this->CI->session->userdata('default_login_lang_id') ? $this->CI->session->userdata('default_login_lang_id') : $this->LANG_ID;
        }
    }
