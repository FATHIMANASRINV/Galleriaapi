...
Class Facebook
{
    ...
    public function __construct()
    {
        ...
        if (!isset($this->fb))
        {
            $this->fb = new FB([
                ...
                'default_graph_version' => $CI->config->item('graph_version', 'facebook')
            ]);
        }

        // Load correct helper depending on login type
        // set in the config file
        switch ($CI->config->item('login_type', 'facebook'))
        {
            case 'js':
        ...
        if ($CI->config->item('auth_on_load', 'facebook') === TRUE)
        {
    ...
    public function login_url()
    {
        ...
        if ($CI->config->item('login_type', 'facebook') != 'web')
        {
            return '';
        }
        return $this->helper->getLoginUrl(
            base_url() . $CI->config->item('login_redirect_url', 'facebook'),
            $CI->config->item('permissions', 'facebook')
        );
    }
    ...
    public function logout_url()
    {
        ...
        return $this->helper->getLogoutUrl(
            ...
            base_url() . $CI->config->item('logout_redirect_url', 'facebook')
        );
    }
